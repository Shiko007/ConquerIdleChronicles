...
class GrindingScene: SKScene {
    ...
    init(size: CGSize, onAddGold: @escaping (Int) -> Void, onAddExp: @escaping (Int) -> Void, onTakeDamage: @escaping (Int) -> Bool, getPlayerAttack: @escaping () -> Int, getPlayerHealth: @escaping () -> Int) {
            ...
            if let targetMonster = arrow.userData?["target"] as? MonsterModel {
                ...
                if distance < 10 {  // Hit threshold (adjust for precision)
                    ...
                    if targetMonster.takeDamage(self.getPlayerAttack()) {
                        ...
                        self.onAddExp(20)  // Example reward
                        self.onAddGold(10)
                    }
